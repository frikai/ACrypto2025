# Copyright 2024 by Matteo Scarlata, Kien Tuong Truong (Applied Cryptography Group, ETH Zurich)
...
class CommandServer(socketserver.StreamRequestHandler):
    ...
    def close_connection(self) -> None:
        ...
        If you want to close the connection from within a command handler immediately
        after calling `close_connection`,you MUST return from the handler. This method
        will not close the connection immediately, but will instead close it at the end
        of the current iteration of the `handle` method.
        """
    ...
    def handle(self) -> None:
        ...
        while self.running:
            ...
            try:
                ...
                self.send_message(
                    {"res": "Failed to execute command: malformed JSON"})
                continue
    ...
    def start_server(cls, host: str, port: int, ipv6: bool = False, **kwargs) -> None:
        ...
        # def cls_injected(request, client_address, server): return cls(
